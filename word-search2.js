    ...
    words.forEach((word)=>{
        ...
        for(let row in board){ 
            ...
            for(let col in board[row]){ 
                ...
                if(!word in wordsMatches) return true;
                const list = new Set()
                const  Aword = word.split("");
                if(checkRecursive(board,Aword,Number(row),Number(col),list)){
                    wordsMatches.push(word)
                    return true
                } 
...
const checkRecursive=(board,word,row,col,list)=>{
    ...
    //go right side
    if(checkRecursive(board,restWord,row,col+1,list)) return true
        //go down
    if(checkRecursive(board,restWord,row+1,col,list)) return true
